...
export default function ProfileDetailPage() {
  ...
  const isCurrentUserProfile = currentUser?.uid === profileId;
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !currentUser || !userProfileRef || !isCurrentUserProfile) return;
  ...
  const handleNameSave = () => {
    if (displayName.trim() && userProfileRef && isCurrentUserProfile) {
  ...
  const handleLocationSave = () => {
    if (displayLocation.trim() && userProfileRef && isCurrentUserProfile) {
  ...
  if (!userProfile || !userProfileRef) {
    // This case should be handled by the notFound call above, but as a fallback
    return (
        <AppLayout>
      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="relative group">
            <Avatar 
                className={`h-24 w-24 border-4 border-primary ${isCurrentUserProfile ? 'cursor-pointer' : ''}`} 
          ...
          <div className="flex-1 space-y-2">
            {isCurrentUserProfile && isNameEditing ? (
            ...
            {isCurrentUserProfile && isLocationEditing ? (
                 <div className="flex items-center gap-2">
        ...
        <FavoriteSportsManager userProfile={userProfile} userProfileRef={userProfileRef} isCurrentUserProfile={isCurrentUserProfile} />
...
function FavoriteSportsManager({ userProfile, userProfileRef, isCurrentUserProfile }: { userProfile: UserProfile, userProfileRef: DocumentReference, isCurrentUserProfile: boolean }) {
        ...
        <Card>
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    ...
                    <CardDescription>
                        {isCurrentUserProfile 
                            ? "Selecteer de sporten die je het liefst beoefent." 
                            : `De favoriete sporten van ${userProfile.name}.`
                        }
                    </CardDescription>
                </div>
                    ...
                    <div className="flex flex-wrap gap-4">
                        {userProfile.favoriteSports && userProfile.favoriteSports.length > 0 ? (
                            ...
                            <p className="text-muted-foreground">
                                {isCurrentUserProfile 
                                    ? "Nog geen favoriete sporten geselecteerd. Klik op 'Wijzig' om ze toe te voegen."
                                    : `${userProfile.name} heeft nog geen favoriete sporten geselecteerd.`
                                }
                            </p>
                        )}
                    </div>
